/* ==========================================================================
   Soft Skill Zone — Inline SVG icon set (Feather-style, 24x24, stroke)
   --------------------------------------------------------------------------
   Inline SVG keeps the site icon-font-free and works offline.
     icon("wallet")                  -> "<svg …>"
     icon("wallet", { size: 20 })
   ========================================================================== */
...
const PATHS = {
  ...
  rupee:   '<path d="M6 3h12M6 8h12M16 21 8 13h2a5 5 0 0 0 0-10"/>',
  ...
  /* Audiobook ke liye — headphone. Study material me PDF aur audio ek hi
     list me hain, isliye pehli nazar me farq dikhna zaroori hai. */
  ...
  whatsapp:'<path d="M20.5 3.5A11.9 11.9 0 0 0 12 0C5.4 0 .1 5.3.1 11.9c0 2.1.6 4.1 1.6 5.9L0 24l6.3-1.7c1.7.9 3.7 1.4 5.7 1.4 6.6 0 11.9-5.3 11.9-11.9 0-3.2-1.2-6.2-3.4-8.3z" fill="currentColor" stroke="none"/>',
  ...
  sparkles: '<path d="m12 3-1.9 5.8L4 10.7l6.1 1.9L12 18.4l1.9-5.8L20 10.7l-6.1-1.9z"/><path d="M18 4v4M20 6h-4M5 17v3M6.5 18.5h-3"/>',
  shield:   '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>',
  target:   '<circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/>',
  briefcase:'<rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/>',
  grid:     '<rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/>',
  /* Class recording ka "bada/chhota karein" — browser ka apna fullscreen
     jaan-boojh kar band hai, wo watermark peechhe chhod deta tha. */
  ...
  qrcode:   '<rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><path d="M14 14h3v3h-3zM18 18h3v3h-3z"/>',
  keyboard: '<rect x="2" y="6" width="20" height="12" rx="2"/><path d="M6 10h.01M10 10h.01M14 10h.01M18 10h.01M8 14h8"/>',
  calculator:'<rect x="4" y="2" width="16" height="20" rx="2"/><path d="M8 6h8M8 10h.01M12 10h.01M16 10h.01M8 14h.01M12 14h.01M16 14h.01M8 18h4M16 18h.01"/>',

  /* Nova ki awaaz — bolkar poochhna, bolkar sunna */
  ...
  volumeOff: '<path d="M11 5 6 9H2v6h4l5 4z"/><path d="m17 9 5 6M22 9l-5 6"/>',
  ...
  telegram: '<path d="M21.5 3.5 2.5 10.8l5.2 1.8 2 6.4 2.8-3.6 4.6 3.4z"/><path d="m7.7 12.6 9.6-6.4-6.6 8.2"/>'
};
...
export function icon(name, opts = {}) {
  const { size = 24, stroke = 1.9, cls = "" } = opts;
  const body = PATHS[name];
  if (!body) {
    console.warn(`[icons] "${name}" nahi mila`);
    return "";
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="${size}" height="${size}"
    ...
    stroke-linejoin="round"${cls ? ` class="${cls}"` : ""} aria-hidden="true" focusable="false">${body}</svg>`;
}

/** Solid-filled variant (stars, ratings). */
export function iconFilled(name, opts = {}) {
  const { size = 24, cls = "" } = opts;
  const body = PATHS[name];
  if (!body) return "";
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="${size}" height="${size}"
    fill="currentColor" stroke="none"${cls ? ` class="${cls}"` : ""} aria-hidden="true">${body}</svg>`;
}
